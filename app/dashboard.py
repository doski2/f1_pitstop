...
if DATA_ROOT is None:
    st.error("No se encontró la carpeta de datos en rutas intentadas: " + '\n'.join(str(p.resolve()) for p in _candidate_paths))
    st.stop()
...
if auto_refresh:
    st.experimental_rerun = getattr(st, 'experimental_rerun', None)  # type: ignore
    st_autorefresh = getattr(st, 'autorefresh', None) or getattr(st, 'experimental_singleton', None)  # fallback
    # Usar la función oficial st.experimental_rerun con st_autorefresh helper
    st_autorefresh = st.experimental_data_editor if False else None  # placeholder to avoid lint; real autorefresh below
    st.autorefresh(interval=15000, key='auto_rfr')  # type: ignore
if do_refresh:
    st.cache_data.clear()
...
with col_meta1:
    st.metric("Total Laps (estimado)", int(lap_summary['currentLap'].max() if not lap_summary.empty else 0))
with col_meta2:
    unique_compounds = ', '.join(sorted(lap_summary['compound'].dropna().unique())) if 'compound' in lap_summary else '—'
...
status_cols[0].markdown(f"**Dos compuestos (seco):** {'✅' if compliance['used_two_compounds'] else '❌'}")
status_cols[1].markdown(f"**Stint razonable:** {'✅' if compliance['max_stint_ok'] else '❌'}")
status_cols[2].markdown(f"**Paradas suficientes:** {'✅' if compliance['pit_stop_required'] else '❌'}")
if compliance['notes']:
    st.warning('\n'.join(compliance['notes']))
st.caption("Reglas simplificadas con fines analíticos; para validación oficial consultar reglamento FIA completo.")

st.subheader("Gráficos")
if not _PLOTLY_AVAILABLE:
    st.warning("Plotly no está instalado. Instale dependencias: `pip install -r requirements.txt`.")
    st.stop()
tab_lap, tab_ttemps, tab_trackair, tab_evol, tab_strategy, tab_wear = st.tabs(["Lap Times", "Tº Neumáticos", "Tº Pista/Aire", "Evolución Compuesto", "Estrategia", "Desgaste"])

with tab_lap:
    if _PLOTLY_AVAILABLE and px is not None and not lap_summary.empty and 'lap_time_s' in lap_summary:
        fig = px.line(lap_summary, x='currentLap', y='lap_time_s', color='compound', markers=True,
        ...
        if 'pit_stop' in lap_summary.columns and lap_summary['pit_stop'].any() and go is not None:
            ...
            fig.add_trace(go.Scatter(x=pit_pts['currentLap'], y=pit_pts['lap_time_s'], mode='markers',
                                     marker=dict(symbol='triangle-down', size=12, color='red'), name='Pit Stop'))
...
with tab_ttemps:
    ...
    if _PLOTLY_AVAILABLE and px is not None and existing:
        melt = df.melt(id_vars=['timestamp', 'currentLap'], value_vars=existing, var_name='Rueda', value_name='Temp')
        fig2 = px.line(melt, x='timestamp', y='Temp', color='Rueda', hover_data=['currentLap'], title='Temperaturas de Neumáticos')
        st.plotly_chart(fig2, use_container_width=True)
    else:
        st.info('No hay columnas de temperatura de rueda.')

with tab_trackair:
    if _PLOTLY_AVAILABLE and go is not None and 'trackTemp' in df.columns and 'airTemp' in df.columns:
        fig3 = go.Figure()
        ...
        fig3.update_layout(title='Temperatura Pista vs Aire', xaxis_title='Tiempo', yaxis_title='°C (según juego)')
...
with tab_evol:
    if _PLOTLY_AVAILABLE and px is not None and not lap_summary.empty:
        fig4 = px.scatter(lap_summary, x='currentLap', y='tire_age', color='compound', size='lap_time_s',
...
with tab_strategy:
    ...
    if practice_data.empty and is_race and not lap_summary.empty:
        if st.checkbox('Sin prácticas. Usar vueltas iniciales de carrera como estimación provisoria'):
    ...
    if practice_data.empty:
        ...
        if col_f2.button('Auto inicial', help='Detectar combustible inicial desde datos') and use_fuel and 'fuel' in practice_data.columns:
        ...
        default_start_fuel = st.session_state['auto_start_fuel'] if st.session_state.get('auto_start_fuel') is not None else (
            float(practice_data['fuel'].max()) if use_fuel and 'fuel' in practice_data.columns and practice_data['fuel'].notna().any() else 100.0
        )
        start_fuel = col_f2.number_input('Combustible inicial (kg)', value=default_start_fuel, min_value=0.0, key='start_fuel_input')
        # Auto-cálculo consumo
        if 'cons_per_lap_override' not in st.session_state:
            st.session_state['cons_per_lap_override'] = None
        if col_f4.button('Auto consumo', help='Estimar consumo medio por vuelta usando diferencias de fuel') and use_fuel and 'fuel' in practice_data.columns:
        ...
        cons_per_lap = col_f3.number_input('Consumo por vuelta (kg)', value=base_cons, min_value=0.0, format='%.3f', key='cons_input')
        if use_fuel and st.session_state.get('cons_per_lap_override') is not None:
            st.caption(f"Consumo auto-estimado: {st.session_state['cons_per_lap_override']:.3f} kg/v")
        ...
        if not models:
            ...
            for comp, coeffs in models.items():
                if len(coeffs) == 3:
                    ...
                    model_rows.append({'Compuesto': comp, 'a': round(a,3), 'b_age': round(b_age,4), 'c_fuel': round(c_fuel,4)})
                else:
                    a, b_age = coeffs  # type: ignore
                    model_rows.append({'Compuesto': comp, 'a': round(a,3), 'b_age': round(b_age,4), 'c_fuel': None})
            ...
            if col_sv1.button('Guardar modelo'):
                sessions_used = sorted(practice_data['session'].unique()) if 'session' in practice_data.columns else []
            ...
            default_track_laps = TRACK_LAPS.get(track, 57)
            completed_laps = int(lap_summary['currentLap'].max()) if not lap_summary.empty else 0
            col_rl1, col_rl2 = st.columns([1,2])
            use_completed = col_rl1.checkbox('Usar vueltas completadas', value=True if is_race else False,
                                             help='Si activo, la estrategia usa sólo las vueltas ya registradas; si lo desactivas puedes planificar toda la distancia.')
            if use_completed:
                race_laps_guess = max(completed_laps, 1)
                col_rl2.markdown(f"**Vueltas para cálculo:** {race_laps_guess} (parcial)")
            else:
                race_laps_guess = col_rl2.number_input('Vueltas totales carrera', value=default_track_laps, min_value=completed_laps if completed_laps>0 else 10, max_value=110)
                if race_laps_guess < completed_laps:
                    st.warning('El total indicado es menor que vueltas ya completadas; se usará número de vueltas completadas.')
                    race_laps_guess = completed_laps
            pit_loss = st.number_input('Pérdida pit stop (s)', value=22.0, min_value=5.0, max_value=60.0, step=0.5)
            ...
            if st.button('Calcular Estrategias'):
                plans = enumerate_plans(race_laps_guess, list(models.keys()), models, practice_data, pit_loss,
                                        max_stops=max_stops, min_stint=min_stint, require_two_compounds=require_two,
                                        use_fuel=use_fuel, start_fuel=start_fuel, cons_per_lap=cons_per_lap)
                if not plans:
                    st.warning('No se generaron planes.')
                else:
                    for i, p in enumerate(plans, 1):
                        st.markdown(f"**Plan {i}** Tiempo total: {p['total_time']:.2f}s | Paradas: {p['stops']}")
                        stint_rows = []
                        acc = 0
                        for idx_s, s in enumerate(p['stints'], 1):
                            ...
                            end_l = acc + s['laps']
                            acc = end_l
                            stint_rows.append({'#': idx_s, 'Compuesto': s['compound'], 'Vueltas': s['laps'], 'Rango': f"{start_l}-{end_l}", 'Tiempo Est. (s)': round(s['pred_time'],2)})
            ...
            if is_race and not lap_summary.empty:
                ...
                current_fuel = float(last_row['fuel'].iloc[0]) if use_fuel and 'fuel' in last_row.columns and not last_row.empty else start_fuel
                if comp_now:
                    rec = live_pit_recommendation(current_lap, race_laps_guess, str(comp_now), age_now, models, practice_data, pit_loss,
                                                  use_fuel=use_fuel, current_fuel=current_fuel, cons_per_lap=cons_per_lap)
                else:
                    rec = None
                if rec:
                    st.success(f"Parar en vuelta {rec['pit_on_lap']} (seguir {rec['continue_laps']}) -> {rec['new_compound']} | Tiempo restante: {rec['projected_total_remaining']:.1f}s")
                else:
...
with tab_wear:
    ...
    if not have_wear:
        ...
        if _PLOTLY_AVAILABLE and px is not None:
            wear_melt = df.melt(id_vars=['timestamp','currentLap'], value_vars=have_wear, var_name='Rueda', value_name='Desgaste')
            figw = px.line(wear_melt, x='currentLap', y='Desgaste', color='Rueda', title='Desgaste Neumáticos (%)', markers=True)
            ...
            if not lap_summary.empty:
                ...
                st.dataframe(last_per_lap[['currentLap']+cols_present], use_container_width=True, height=300)
        else:
            st.info('Plotly no disponible para graficar desgaste.')

st.sidebar.markdown('---')
st.sidebar.caption('Versión inicial. Mejora sugerida: simulador de degradación y comparación de estrategias alternativas.')
