...
with tab_strategy:
    ...
    if practice_data.empty:
        ...
        if col_f4.button('Auto consumo', help='Estimar consumo medio por vuelta usando diferencias de fuel') and use_fuel and 'fuel' in practice_data.columns:
            ...
            if len(fuel_series) >= 5:
                ...
                if len(plausible) >= 3:
                    ...
                    st.warning('Insuficientes diferencias plausibles de fuel para estimar consumo; usando valor base.')
        base_cons = 1.4
        ...
        if not models:
            ...
            TRACK_LAPS = {
                'Bahrain': 57, 'Monaco': 78, 'Monza': 53, 'Spa': 44, 'Silverstone': 52,
                'Barcelona': 66, 'Suzuka': 53, 'Canada': 70, 'Brazil': 71, 'Abu Dhabi': 58,
                'SaudiArabia': 50,
            }
            ...
            use_completed = col_rl1.checkbox(
                ...
                help='Si activo, los planes se calculan solo con lo ya corrido (parcial). La recomendación en vivo usa la distancia total oficial.'
            )
            if use_completed:
            ...
            if st.button('Calcular Estrategias'):
                plans = enumerate_plans(race_laps_horizon, list(models.keys()), models, practice_data, pit_loss,
            ...
            if is_race and not lap_summary.empty:
                ...
                if comp_now:
                    rec = live_pit_recommendation(current_lap, total_race_laps_real, str(comp_now), age_now, models, practice_data, pit_loss,
...
with tab_wear:
    ...
    if not have_wear:
        st.info('No hay columnas de desgaste (flDeg, frDeg, rrDeg).')
    else:
...
st.sidebar.markdown('---')
st.sidebar.caption('Versión modular. Próximos pasos: métricas de modelo, simulación Safety Car, comparación multi-piloto.')
